...
def pre_process_images(X: np.ndarray):
    ...
    intial_intervall_length = 255
    final_interval_length = 2
    batch_size = X.shape[0]

    # Normalization anb bias trick
    result = X/(intial_intervall_length/final_interval_length) - 1
    result = np.block([result,np.ones([batch_size,1])])
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    """
    Cost = np.mean(-(targets*np.log(outputs) + (1-targets)*np.log(1-outputs)),axis=0)
    # Normalize with respect to batch size (?)
